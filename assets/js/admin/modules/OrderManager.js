export default class OrderManager {
    ...
    async updateStatus(id, idEstado) {
        try {
            const data = await api.post('pedidos.php?action=cambiarEstado', { id_pedido: id, id_estado: idEstado });
    ...
    async deleteOrder(id) {
        ...
        if (confirmar.isConfirmed) {
            try {
                const data = await api.post('pedidos.php?action=eliminar', { id_pedido: id });
    ...
    async showDetails(id) {
        try {
            const lineas = await api.get(`pedidos.php?id=${id}`);
